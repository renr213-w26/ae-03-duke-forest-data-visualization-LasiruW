...
ggplot(data = duke_forest, aes(x = price)) + 
  ...
  x = "Price($)" , 
  y = "Count",
  title = "How Garages Impact Home Price Distribution",)

```

Add narrative here...
The presence of a garage appears to marginally lower prices.
Properties with no garages may be more in demand.
...
ggplot(data = duke_forest, aes(x = area, y = price, color = year_built)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  labs(
    title = "How Size and Year of Construction Impacts House Prices",
    x = "Area(thousand acre)",
    y = "Price(million $)",
    color = "Year Built"
  ) +
  scale_y_continuous(label = label_number(scale = 1/100000)) +
  scale_x_continuous(label = label_number(scale = 1/1000))
```

Larger houses are priced higher while newer homes are not priced higher.
Overall, time of construction does not impact pricing as newer and older homes are priced across the board.
Property size does have an impact on price.
